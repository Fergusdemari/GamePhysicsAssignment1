...
class CustomMenu : public igl::opengl::glfw::imgui::ImGuiMenu
{
	...
	virtual void draw_viewer_menu() override
	{
		...
		if (ImGui::CollapsingHeader("Algorithm Options", ImGuiTreeNodeFlags_DefaultOpen))
		{
            ...
            ImGui::InputFloat("Drag Coeff", &DragCoeff);
			if (ImGui::InputFloat("Time Step", &timeStep)) {
				mgpViewer.core.animation_max_fps = (((int)1.0 / timeStep));
			}
		}

		if (ImGui::CollapsingHeader("Changing Velocity")) {
			static int item_current = 0;
			if (ImGui::Combo("Object ID", &item_current, scene.directions)) {
...
int main(int argc, char *argv[])
{
	...
	//create platform
	createPlatform();
	RowVector3d vel = RowVector3d(0, 0, 0);
	scene.addMesh(platV, platF, platT, 10000.0, true, platCOM, platOrientation, vel);
