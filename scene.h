...
public:
	...
	Mesh(const MatrixXd& _V, const MatrixXi& _F, const MatrixXi& _T, const double density, const bool _isFixed, const RowVector3d& _COM, const RowVector4d& _orientation, const RowVector3d& _velocity) {
		origV = _V;
		F = _F;
		T = _T;
		isFixed = _isFixed;
...
public:
	...
	void addMesh(const MatrixXd& V, const MatrixXi& F, const MatrixXi& T, const double density, const bool isFixed, const RowVector3d& COM, const RowVector4d& orientation, const RowVector3d& velocity) {

		Mesh m(V, F, T, density, isFixed, COM, orientation, velocity);
		meshes.push_back(m);
	}
	...
	void handleCollision(Mesh& m1, Mesh& m2, const double& depth, const RowVector3d& contactNormal, const RowVector3d& penPosition, const double CRCoeff) {
		...
		long double m1InvMass = 1 / m1.totalMass;
		long double m2InvMass = 1 / m2.totalMass;
		if (m1.isFixed) {
		...
		//Vector3d augA1 = ();
		long double aug1 = rCrossN1.transpose() * m1.getCurrInvInertiaTensor() * r1.cross(contactNormal);

		long double aug2 = rCrossN2.transpose() * m2.getCurrInvInertiaTensor() * r2.cross(contactNormal);

		Vector3d velocityDiff = v2 - v1;
		long double diffDotNorm = velocityDiff.dot(contactNormal);

		long double j = (-(1 + CRCoeff) * diffDotNorm) / (m1InvMass + m2InvMass + aug1 + aug2);

		Vector3d impulse = j * contactNormal;  //change this to your result

		if (impulse.norm() > 0.0001) {
	...
	bool loadScene(const std::string dataFolder, const std::string sceneFileName) {
		...
		for (int i = 0; i < numofObjects; i++) {
			...
			cout << sceneFileHandle.peek() << endl;
			while (sceneFileHandle.peek() == 32) {
				sceneFileHandle.get();
			}
			if (sceneFileHandle.peek() != -1 && sceneFileHandle.peek() != 10) {
				sceneFileHandle >> velocity(0) >> velocity(1) >> velocity(2);
				cout << "Velocity: " << velocity << endl;
			}
			else {
				velocity = RowVector3d(0, 0, 0);

			}
			//string vel = to_string(velocity(0)) + ", " + to_string(velocity(1)) + ", " + to_string(velocity(2));

			directions.push_back(to_string(i));
			...
			addMesh(objV, objF, objT, density, isFixed, userCOM, userOrientation, velocity);
